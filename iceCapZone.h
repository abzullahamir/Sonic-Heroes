...
class iceCapZone : public Levels {
private:
    ...
    static const int maxCollectibles = 164;
...
public:
    iceCapZone() {
        ...
        backgroundTexture.loadFromFile("Data/bg2.jpg");
    ...
    void loadLevel() {
        ...
        grid->loadFromFile("Data/level2.txt");
        loadCollectibles();
        loadEnemies();
    }
    ...
    void spawnPlayers(player* arrPlayers[]) override {
        for (int i = 0; i < numPlayers; ++i) {
            ...
            arrPlayers[i]->setDec(0.70f);
            arrPlayers[i]->setGrav(0.f);
        }
    }
    ...
    void render(RenderWindow& window, float offsetX) {
        ...
        // draw tiles
        grid->render(window, offsetX,
            wallSprite, breakableSprite, springSprite,
            crystalSprite, platformSprite, spikeSprite);
    }
